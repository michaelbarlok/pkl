...
export async function GET(request: NextRequest) {
  ...
  const { data: sheets } = await supabase
    .from("signup_sheets")
    .select("id, event_date, event_time, group_id, group:shootout_groups(name)")
  ...
  for (const sheet of sheets) {
    ...
    if (playerIds.length > 0) {
      ...
      const eventDateStr = sheet.event_date ?? "";
      const eventTimeStr = sheet.event_time ?? "";

      await notifyMany(playerIds, {
        type: "session_starting",
        title: `Session tomorrow: ${gName}`,
        body: `You're confirmed for ${gName} on ${formatDate(eventDateStr)}${eventTimeStr ? ` at ${formatTime(eventTimeStr)}` : ""}. Can't make it? Please withdraw so someone on the waitlist can play.`,
